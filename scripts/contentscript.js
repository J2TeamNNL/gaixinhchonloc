...
var link_photo, width, height, div_parent, div;
...
<<<<<<< HEAD
  jQuery(document).ready(function ($) {
    ...
    $(window).scroll(function () {
      // bài chia sẻ
      // let div = $('[data-testid="Keycommand_wrapper_feed_attached_story"]').closest(`div[data-pagelet^="FeedUnit_"]`).first();
...
=======
	...
	jQuery(document).ready(function ($) {
		chrome.storage.local.get("opts", ({opts: opts}) => {console.log(opts);}); // Get value of options
		$('[aria-label="Marketplace"]').closest("li").remove();
		$(window).scroll(function () {
			var div = $('[aria-label="Sponsored"]').closest("div[data-pagelet^=\"FeedUnit_\"]").first();
>>>>>>> 9d55901a5c44c16bbdbea49cbc632bd3667c427e
...
<<<<<<< HEAD
      if (div.length != 0) {
        ...
        changePagePicture(); //    chỉnh tên Page

        changePageName(); // tìm div thông tin bài đăng

        div_parent = div.find("a[aria-hidden=\"true\"]").closest('div:not([class])').next(); // xoá hết thông tin bài đăng gốc
...
function changePageName() {
  ...
  div.find('strong').parent('a').attr('href', link_page);
}
