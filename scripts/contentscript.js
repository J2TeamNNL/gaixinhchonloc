'use strict';
let link_avatar = `https://scontent.fhan2-2.fna.fbcdn.net/v/t1.0-1/cp0/c0.0.80.80a/p80x80/91871764_1323740761159860_7711545584477274112_o.jpg?_nc_cat=111&_nc_sid=1eb0c7&_nc_ohc=AQvmZY8hqIYAX_vhFf9&_nc_ht=scontent.fhan2-2.fna&oh=4c5a556c869fefc8b00bb1ccdad3a674&oe=5F65FFD6`;
let link_api = `https://gxcl.info/api.php`;
let link_page = `https://facebook.com/gaixinhchonloc`;
let link_photo, width, height, div;
let ads = false, shared_post = false, store = false, contains_keywords = false, contains_name = false, array_keywords = [], array_name = [];
        ...
        $(window).scroll(function () {
            ...
            if(contains_keywords){
                ...
                for(var i = 0; i < array_keywords.length; i++){
                    ...
                    if (div.length == 0) {
                        $(`div[class="${class_comment_text}"]:contains(${array_keywords[i]}):not('.ex-replaced')`).html(`
                            ${chrome.i18n.getMessage('comment')} ${chrome.i18n.getMessage('notification')} 
...
function getDivPost(keyword){
    ...
    for (var i = 0; i < class_post_text.length; i++){
        let div = $(`div[data-pagelet^="FeedUnit"]`).has(`div[class="${class_post_text[i]}"]`).regex(keyword).first();
        if(div.length!=0){
...
function getDivGroup(name){
    ...
    for (var i = 0; i < class_div_name.length; i++){
        let div = $(`div[data-pagelet^="FeedUnit"]`).has(`div[class="${class_div_name[i]}"]`).regex(name).first();
        if(div.length!=0){
